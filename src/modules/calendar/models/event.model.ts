...
// Static methods
CalendarEventSchema.statics.findByCalendar = function (
  calendarId: string,
  startDate?: Date,
  endDate?: Date
) {
...
CalendarEventSchema.statics.findByDateRange = function (
  ...
  const query: any = {
    $and: [{ endTime: { $gte: startDate } }, { startTime: { $lte: endDate } }]
  };
...
// Instance methods
CalendarEventSchema.methods.addAttendee = function (
  attendee: Omit<IEventAttendee, 'responseTime'>
) {
...
CalendarEventSchema.methods.updateAttendeeStatus = function (
  email: string,
  status: IEventAttendee['status']
) {
...
export const CalendarEventModel = model<ICalendarEventDocument, ICalendarEventModel>(
  'CalendarEvent',
  CalendarEventSchema
);
