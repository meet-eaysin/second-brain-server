...
const initializeCrossModuleRelations = async (userId: string): Promise<void> => {
  ...
  for (const module of userModules) {
    ...
    if (moduleConfig?.defaultRelations) {
      await setupModuleRelations(module, moduleConfig.defaultRelations, userId);
    }
  }
};

const setupModuleRelations = async (
  sourceModule: any,
...
const getModuleRelationStats = async (
  ...
  for (const record of records) {
    const relatedRecords = await getRelatedRecords(record.id.toString());
...
const getRelationInsights = async (userId: string): Promise<IRelationInsights> => {
  const userModules = await getUserModules(userId);
  ...
  for (const module of userModules) {
    const stats = await getModuleRelationStats(module.type, userId);
...
> => {
  ...
  for (const moduleType of targetModules) {
    ...
    for (const targetRecord of targetRecords) {
      const targetText = extractRecordText(targetRecord);
      ...
      const similarityScore = calculateSimilarity(recordText, targetText, recordTags, targetTags);
      ...
      if (similarityScore >= threshold) {
        suggestions.push({
          ...
          similarityScore,
          suggestedRelationType: suggestRelationType(record, targetRecord),
          reasoning: generateReasoningText(similarityScore, recordTags, targetTags)
        });
      }
    }
  }
...
const extractRecordText = (record: any): string => {
  const textFields = [];
...
const suggestRelationType = (record1: any, record2: any): string => {
...
export const crossModuleRelationsService = {
  initializeCrossModuleRelations,
  setupModuleRelations,
  connectRecords,
  disconnectRecords,
  getRelatedRecords,
