...
export class FormulaFunctionsService {
  private functions: Map<string, IFormulaFunction> = new Map();
  ...
  private registerBuiltInFunctions(): void {
    ...
    this.registerFunction({
      ...
      syntax: 'ABS(number)',
      parameters: [
        { name: 'number', type: EFormulaDataType.NUMBER, description: 'The number to get absolute value of' }
      ],
    ...
    this.registerFunction({
      ...
      parameters: [
        ...
        { name: 'decimals', type: EFormulaDataType.NUMBER, description: 'Number of decimal places', isOptional: true, defaultValue: 0 }
      ],
      returnType: EFormulaDataType.NUMBER,
      examples: [
        { expression: 'ROUND(3.14159, 2)', description: 'Round to 2 decimal places', result: 3.14 },
    ...
    this.registerFunction({
      ...
      examples: [
        ...
        { expression: 'SUM([Price], [Tax])', description: 'Sum of Price and Tax properties', result: 'varies' }
      ]
    ...
    this.registerFunction({
      ...
      returnType: EFormulaDataType.NUMBER,
      examples: [
        { expression: 'MIN(5, 2, 8, 1)', description: 'Minimum of 5, 2, 8, 1', result: 1 }
      ]
    ...
    this.registerFunction({
      ...
      returnType: EFormulaDataType.NUMBER,
      examples: [
        { expression: 'MAX(5, 2, 8, 1)', description: 'Maximum of 5, 2, 8, 1', result: 8 }
      ]
    ...
    this.registerFunction({
      ...
      parameters: [
        { name: 'texts', type: [EFormulaDataType.TEXT], description: 'Text strings to concatenate' }
      ],
      returnType: EFormulaDataType.TEXT,
      examples: [
        { expression: 'CONCAT("Hello", " ", "World")', description: 'Concatenate strings', result: 'Hello World' }
      ]
    ...
    this.registerFunction({
      ...
      syntax: 'UPPER(text)',
      parameters: [
        { name: 'text', type: EFormulaDataType.TEXT, description: 'Text to convert' }
      ],
      returnType: EFormulaDataType.TEXT,
      examples: [
    ...
    this.registerFunction({
      ...
      syntax: 'LEN(text)',
      parameters: [
        { name: 'text', type: EFormulaDataType.TEXT, description: 'Text to measure' }
      ],
      returnType: EFormulaDataType.NUMBER,
      examples: [
        { expression: 'LEN("Hello")', description: 'Length of "Hello"', result: 5 }
      ]
    ...
    this.registerFunction({
      ...
      returnType: EFormulaDataType.TEXT,
      examples: [
        { expression: 'LEFT("Hello World", 5)', description: 'First 5 characters', result: 'Hello' }
      ]
    ...
    this.registerFunction({
      ...
      returnType: EFormulaDataType.TEXT,
      examples: [
        { expression: 'RIGHT("Hello World", 5)', description: 'Last 5 characters', result: 'World' }
      ]
    }, (args: any[]) => {
    ...
    this.registerFunction({
      ...
      examples: [
        { expression: 'NOW()', description: 'Current date and time', result: new Date() }
      ]
    }, () => new Date());
    ...
    this.registerFunction({
      ...
      returnType: EFormulaDataType.DATE,
      examples: [
        { expression: 'TODAY()', description: 'Current date', result: new Date() }
      ]
    }, () => {
    ...
    this.registerFunction({
      ...
      returnType: EFormulaDataType.NUMBER,
      examples: [
        { expression: 'YEAR(TODAY())', description: 'Current year', result: new Date().getFullYear() }
      ]
    ...
    this.registerFunction({
      ...
      returnType: EFormulaDataType.NUMBER,
      examples: [
        { expression: 'MONTH(TODAY())', description: 'Current month', result: new Date().getMonth() + 1 }
      ]
    ...
    this.registerFunction({
      ...
      parameters: [
        ...
        { name: 'value_if_true', type: EFormulaDataType.ANY, description: 'Value when condition is true' },
        { name: 'value_if_false', type: EFormulaDataType.ANY, description: 'Value when condition is false' }
      ],
      returnType: EFormulaDataType.ANY,
      examples: [
        { expression: 'IF([Score] >= 90, "A", "B")', description: 'Grade based on score', result: 'varies' }
      ]
    ...
    this.registerFunction({
      ...
      parameters: [
        { name: 'conditions', type: [EFormulaDataType.BOOLEAN], description: 'Conditions to test' }
      ],
    ...
    this.registerFunction({
      ...
      returnType: EFormulaDataType.BOOLEAN,
      examples: [
        { expression: 'NOT(true)', description: 'Opposite of true', result: false }
      ]
    }, (args: any[]) => !Boolean(args[0]));
    ...
    this.registerFunction({
      ...
      parameters: [
        { name: 'value', type: EFormulaDataType.ANY, description: 'Value to check' }
      ],
      returnType: EFormulaDataType.BOOLEAN,
      examples: [
        { expression: 'ISBLANK("")', description: 'Is empty string blank?', result: true }
      ]
    }, (args: any[]) => {
      const value = args[0];
    ...
    this.registerFunction({
      ...
      returnType: EFormulaDataType.BOOLEAN,
      examples: [
        { expression: 'ISNUMBER(42)', description: 'Is 42 a number?', result: true }
      ]
  ...
  registerFunction(definition: IFormulaFunction, executor: Function): void {
    ...
    this.executors.set(definition.name.toUpperCase(), executor);

    // Register aliases
    if (definition.aliases) {
      definition.aliases.forEach(alias => {
        this.functions.set(alias.toUpperCase(), definition);
        this.executors.set(alias.toUpperCase(), executor);
      });
    }
  }
  ...
  executeFunction(name: string, args: any[], context?: IFormulaContext): any {
    const executor = this.executors.get(name.toUpperCase());
    if (!executor) {
    ...
    try {
      ...
      throw new Error(`Error executing function ${name}: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  validateFunctionCall(name: string, args: any[]): { isValid: boolean; errors: string[] } {
    ...
    if (args.length < requiredParams.length) {
      errors.push(`Function ${name} requires at least ${requiredParams.length} arguments, got ${args.length}`);
    }

    if (args.length > func.parameters.length && !func.parameters.some(p => Array.isArray(p.type))) {
      errors.push(`Function ${name} accepts at most ${func.parameters.length} arguments, got ${args.length}`);
    }
...
export const formulaFunctionsService = new FormulaFunctionsService();
