...
export const getContentQuerySchema = z.object({
  ...
  type: z.array(z.nativeEnum(EContentType)).or(
    z.string().transform(val => val.split(',').map(s => s.trim() as EContentType))
  ).optional(),
  status: z.array(z.nativeEnum(EContentStatus)).or(
    z.string().transform(val => val.split(',').map(s => s.trim() as EContentStatus))
  ).optional(),
  priority: z.array(z.nativeEnum(EContentPriority)).or(
    z.string().transform(val => val.split(',').map(s => s.trim() as EContentPriority))
  ).optional(),
  stage: z.array(z.nativeEnum(EWorkflowStage)).or(
    z.string().transform(val => val.split(',').map(s => s.trim() as EWorkflowStage))
  ).optional(),
  categories: z.array(z.string()).or(z.string().transform(val => val.split(','))).optional(),
  ...
  scheduledAfter: z.string().datetime().transform(val => new Date(val)).optional(),
  scheduledBefore: z.string().datetime().transform(val => new Date(val)).optional(),
  publishedAfter: z.string().datetime().transform(val => new Date(val)).optional(),
  publishedBefore: z.string().datetime().transform(val => new Date(val)).optional(),
  ...
  sortBy: z.enum(['title', 'createdAt', 'updatedAt', 'scheduledDate', 'publishedDate', 'priority']).default('updatedAt'),
...
export const scheduleContentSchema = z.object({
  scheduledDate: z.string().datetime().transform(val => new Date(val)),
...
export const createTemplateSchema = z.object({
  ...
  structure: z.object({
    sections: z.array(z.object({
      ...
      placeholder: z.string().min(1, 'Placeholder is required').max(200, 'Placeholder too long'),
...
export const updateTemplateSchema = createTemplateSchema.omit({ name: true }).partial().extend({
...
export const createCalendarSchema = z.object({
  ...
  publishingSchedule: z.array(z.object({
...
export const updateCalendarSchema = createCalendarSchema.omit({ name: true }).partial().extend({
...
export const contentAnalyticsQuerySchema = z.object({
  ...
  includeMetrics: z.boolean().or(z.string().transform(val => val === 'true')).default(true)
});
