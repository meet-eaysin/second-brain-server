...
export class ContentService {
  ...
  async createContent(data: ICreateContentRequest, userId: string): Promise<IContentPiece> {
    try {
      ...
      const contentRecord = new RecordModel({
        ...
        content: this.createContentBlocks(data.content, userId),
        createdBy: userId,
        updatedBy: userId,
      ...
      return this.formatContentResponse(savedRecord);
    } catch (error: any) {
  ...
  async getContent(params: IContentQueryParams, userId: string): Promise<{
    ...
    try {
      const query = this.buildContentQuery(params, userId);
      ...
      const formattedContent = content.map(item => this.formatContentResponse(item));
  ...
  async getContentById(id: string, userId: string): Promise<IContentPiece> {
    try {
  ...
  async updateContent(id: string, data: IUpdateContentRequest, userId: string): Promise<IContentPiece> {
    try {
      const content = await RecordModel.findOne({
        _id: id,
        isDeleted: { $ne: true }
      ...
      if (data.content !== undefined) {
        ...
        updateData['properties.Word Count'] = this.calculateWordCount(data.content);
        updateData['properties.Reading Time'] = this.calculateReadingTime(updateData['properties.Word Count']);
      }
      ...
      if (data.metaDescription !== undefined) updateData['properties.Meta Description'] = data.metaDescription;
      ...
      if (data.scheduledDate !== undefined) updateData['properties.Scheduled Date'] = data.scheduledDate;
      ...
      if (data.relatedContentIds !== undefined) updateData['properties.Related Content IDs'] = data.relatedContentIds;
      if (data.sourceNoteIds !== undefined) updateData['properties.Source Note IDs'] = data.sourceNoteIds;
      if (data.sourceResourceIds !== undefined) updateData['properties.Source Resource IDs'] = data.sourceResourceIds;
      if (data.featuredImage !== undefined) updateData['properties.Featured Image'] = data.featuredImage;
      ...
      if (data.approvalNotes !== undefined) updateData['properties.Approval Notes'] = data.approvalNotes;
      ...
      if (data.allowComments !== undefined) updateData['properties.Allow Comments'] = data.allowComments;
      if (data.requireApproval !== undefined) updateData['properties.Require Approval'] = data.requireApproval;
      if (data.customFields !== undefined) updateData['properties.Custom Fields'] = data.customFields;

      const updatedContent = await RecordModel.findByIdAndUpdate(
        id,
        updateData,
  ...
  async deleteContent(id: string, userId: string, permanent: boolean = false): Promise<void> {
    try {
      const content = await RecordModel.findOne({
        _id: id,
        isDeleted: { $ne: true }
  ...
  private buildContentQuery(params: IContentQueryParams, userId: string): any {
    const query: any = {
      isDeleted: { $ne: true }
    };
  ...
  private formatContentResponse(record: any): IContentPiece {
    return {
      ...
      content: this.extractPlainTextFromContent(record.content || []),
      ...
      allowComments: record.properties['Allow Comments'] !== undefined ? record.properties['Allow Comments'] : true,
   ...
   */
  private createContentBlocks(content: string | undefined, userId: string): IRecordContent[] {
   ...
   */
  private extractPlainTextFromContent(content: IRecordContent[]): string {
    return content
      .map(block =>
        block.content
