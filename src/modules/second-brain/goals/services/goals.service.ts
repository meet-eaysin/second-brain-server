 ...
 * Create a new goal
 */
async function createGoal(data: ICreateGoalRequest, userId: string): Promise<IGoal> {
  try {
    ...
    const nextReviewDate = calculateNextReviewDate(data.reviewFrequency);
    ...
    if (data.parentGoalId) {
      await addSubGoal(data.parentGoalId, (savedRecord._id as any).toString(), userId);
    }
    ...
    return formatGoalResponse(savedRecord);
  } catch (error: any) {
...
async function getGoals(
  ...
  try {
    const query = buildGoalQuery(params, userId);
    ...
    const formattedGoals = goals.map(goal => formatGoalResponse(goal));
    ...
    if (params.includeStats) {
      stats = await getGoalStats(userId, params.databaseId);
    }
 ...
 * Get a goal by ID
 */
async function getGoalById(id: string, userId: string): Promise<IGoal> {
  try {
 ...
 * Update a goal
 */
async function updateGoal(id: string, data: IUpdateGoalRequest, userId: string): Promise<IGoal> {
  try {
    const goal = await RecordModel.findOne({
      _id: id,
      isDeleted: { $ne: true }
    ...
    const updatedGoal = await RecordModel.findByIdAndUpdate(id, updateData, {
      new: true,
      runValidators: true
    }).exec();
    ...
    await DatabaseModel.findByIdAndUpdate(updatedGoal.databaseId, { lastActivityAt: new Date() });
 ...
 * Delete a goal
 */
async function deleteGoal(id: string, userId: string, permanent: boolean = false): Promise<void> {
  try {
    const goal = await RecordModel.findOne({
      _id: id,
      isDeleted: { $ne: true }
    ...
    await DatabaseModel.findByIdAndUpdate(goal.databaseId, {
      $inc: { recordCount: -1 },
      lastActivityAt: new Date()
    });
  } catch (error: any) {
...
/**
 * Build MongoDB query for goals
 */
function buildGoalQuery(params: IGoalQueryParams, userId: string): any {
  const query: any = {
    isDeleted: { $ne: true }
  };
...
/**
 * Format database record to goal interface
 */
function formatGoalResponse(record: any): IGoal {
 ...
 */
function calculateNextReviewDate(frequency?: 'weekly' | 'monthly' | 'quarterly'): Date | undefined {
 ...
 */
async function addSubGoal(parentGoalId: string, subGoalId: string, userId: string): Promise<void> {
  await RecordModel.findByIdAndUpdate(parentGoalId, {
 ...
 * Get goal statistics
 */
async function getGoalStats(userId: string, databaseId?: string): Promise<IGoalStats> {
...
/**
 * Goals service object containing all goal-related operations
 */
