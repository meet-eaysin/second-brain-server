...
import { INoteBlockComment, INoteCollaborator, ECollaboratorRole } from '../types/notes.types';
import {
  createAppError,
  createNotFoundError,
...
export const addComment = async (
  ...
  try {
    ...
    // Add comment to note
    const updatePath = data.blockId ? 'content.$[block].comments' : 'comments';
    ...
    const arrayFilters = data.blockId ? [{ 'block.id': data.blockId }] : undefined;

    await RecordModel.findByIdAndUpdate(noteId, updateQuery, { arrayFilters });

    // Send notification to note owner and other collaborators
    await sendCommentNotification(noteId, comment, userId);
  ...
  try {
    ...
    if (!found && note.content) {
      for (let i = 0; i < note.content.length; i++) {
        const block = note.content[i] as any;
        if (block.comments) {
          const commentIndex = block.comments.findIndex(
            (c: INoteBlockComment) => c.id === commentId
          );
          if (commentIndex !== -1) {
...
export const addReaction = async (
  ...
  try {
    ...
    await RecordModel.findOneAndUpdate(
      {
        _id: noteId,
        $or: [{ 'comments.id': commentId }, { 'content.comments.id': commentId }]
      },
      {
        $push: {
 ...
 * Unresolve a comment
 */
export const unresolveComment = async (
  noteId: string,
  commentId: string,
  userId: string
): Promise<void> => {
...
export const shareNote = async (
  ...
  try {
    ...
    await sendSharingNotifications(noteId, data.userIds, data.message, userId);
  } catch (error: any) {
    if (error.statusCode) throw error;
...
const sendCommentNotification = async (
  noteId: string,
  comment: INoteBlockComment,
  userId: string
): Promise<void> => {
  try {
    const { createNotification } = await import('@/modules/system/services/notifications.service');
    const { ENotificationType, ENotificationPriority } = await import(
      '@/modules/system/types/notifications.types'
    );
 ...
 * Send sharing notifications
 */
const sendSharingNotifications = async (
  noteId: string,
  userIds: string[],
