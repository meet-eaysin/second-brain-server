...
export class NotesService {
  ...
  async createNote(data: ICreateNoteRequest, userId: string): Promise<INote> {
    try {
      ...
      if (data.content && Array.isArray(data.content) && data.content.length > 0) {
        ...
        richContent = this.convertNoteContentToRecordContent(data.content, userId);
      } else {
      ...
      const contentForMetrics = data.content && Array.isArray(data.content) && data.content.length > 0
        ? data.content
      ...
      const noteRecord = new RecordModel({
        ...
        lastEditedBy: userId,
        lastEditedAt: new Date(),
        autoTags: this.extractAutoTags(textContent),
      ...
      return this.formatNoteResponse(savedRecord);
    } catch (error: any) {
  ...
  async getNotes(params: INoteQueryParams, userId: string): Promise<{
    notes: INote[];
    total: number;
    page: number;
    limit: number;
    ...
    try {
      const query = this.buildNoteQuery(params, userId);
      ...
      const formattedNotes = notes.map(note => this.formatNoteResponse(note));
      ...
      if (params.includeStats) {
        stats = await this.getNoteStats(userId, params.databaseId);
      }
  ...
  async updateNote(id: string, data: IUpdateNoteRequest, userId: string): Promise<INote> {
    try {
      const note = await RecordModel.findOne({
        _id: id,
        isDeleted: { $ne: true }
  ...
  async updateNoteContent(id: string, data: IUpdateNoteContentRequest, userId: string): Promise<INote> {
    try {
      const note = await RecordModel.findOne({
        _id: id,
        isDeleted: { $ne: true }
  ...
  async deleteNote(id: string, userId: string, permanent: boolean = false): Promise<void> {
    try {
      const note = await RecordModel.findOne({
        _id: id,
        isDeleted: { $ne: true }
  ...
  private buildNoteQuery(params: INoteQueryParams, userId: string): any {
    const query: any = {
      isDeleted: { $ne: true }
    };
   ...
   */
  private convertNoteContentToRecordContent(noteContent: INoteContentBlock[], userId: string): IRecordContent[] {
    return noteContent.map(block => {
      // Convert IRichTextElement[] to IRichText[] with proper type mapping
      const richTextContent: IRichText[] = block.content.map(element => this.convertRichTextElement(element));

      return {
        id: block.id,
        type: block.type,
        content: richTextContent,
        children: block.children?.map(child => this.convertNoteContentToRecordContent([child], userId)[0]),
        checked: block.checked,
        language: block.language,
        caption: block.caption ? block.caption.map(element => this.convertRichTextElement(element)) : undefined,
        url: block.url,
   ...
   * Convert IRichTextElement to IRichText with proper type mapping
   */
  private convertRichTextElement(element: any): IRichText {
  ...
  private formatNoteResponse(record: any): INote {
    // Extract custom fields (exclude predefined properties)
    const predefinedProperties = new Set([
      'Title', 'Summary', 'Tags', 'Word Count', 'Reading Time', 'Last Viewed At',
      'View Count', 'Linked Notes', 'Backlinks', 'Mentions', 'Is Published',
      'Published At', 'Is Bookmarked', 'Shared With', 'Allow Comments',
      'Auto Generated Summary', 'Extracted Keywords', 'Suggested Tags', 'Rich Content'
    ]);
  ...
  private extractAutoTags(content: string): string[] {
    ...
    const keywords = [
      'meeting', 'project', 'idea', 'todo', 'important', 'urgent',
      'research', 'design', 'development', 'bug', 'feature', 'review'
    ];
  ...
  private async getNoteStats(userId: string, databaseId?: string): Promise<INoteStats> {
    const query: any = {
      isDeleted: { $ne: true },
      createdBy: userId
    };
