...
export const createNoteTemplate = async (
  ...
  try {
    ...
    return formatTemplateResponse(savedTemplate);
  } catch (error: any) {
...
/**
 * Get note templates with pagination and filtering
 */
export const getNoteTemplates = async (
  ...
  try {
    const query = buildTemplateQuery(params, userId);
    ...
    const [templates, total] = await Promise.all([
      TemplateModel.find(query).sort(sortOptions).skip(skip).limit(limit).exec(),
      TemplateModel.countDocuments(query)
    ]);

    const formattedTemplates = templates.map(template => formatTemplateResponse(template));
 ...
 */
export const getNoteTemplateById = async (id: string, userId: string): Promise<INoteTemplate> => {
  try {
...
export const updateNoteTemplate = async (
  ...
  try {
    ...
    const updatedTemplate = await TemplateModel.findByIdAndUpdate(id, updateData, {
      new: true,
      runValidators: true
    }).exec();
...
/**
 * Apply a note template to create a new note
 */
export const applyNoteTemplate = async (
  templateId: string,
  data: {
    databaseId: string;
    title?: string;
  ...
  try {
    const template = await getNoteTemplateById(templateId, userId);
    ...
    const noteData: ICreateNoteRequest = {
      ...
      content: processTemplateContent(template.content, data.customValues || {}),
      tags: template.tags,
      isPublished: false,
      allowComments: true
    };

    return await createNote(noteData, userId);
  } catch (error: any) {
    if (error.statusCode) throw error;
...
/**
 * Duplicate a note template
 */
export const duplicateNoteTemplate = async (
  id: string,
  data: {
    name?: string;
    isPublic?: boolean;
  ...
  try {
    const originalTemplate = await getNoteTemplateById(id, userId);
    ...
    return await createNoteTemplate(duplicateData, userId);
  } catch (error: any) {
    if (error.statusCode) throw error;
...
/**
 * Get popular note templates
 */
export const getPopularNoteTemplates = async (limit: number = 10): Promise<INoteTemplate[]> => {
  try {
    ...
    return templates.map(template => formatTemplateResponse(template));
  } catch (error: any) {
 ...
 */
export const getFeaturedNoteTemplates = async (limit: number = 10): Promise<INoteTemplate[]> => {
  try {
    const templates = await TemplateModel.find({
      type: 'note',
      isPublic: true,
...
/**
 * Build template query from parameters
 */
const buildTemplateQuery = (params: any, userId: string): any => {
  const query: any = {
    type: 'note',
    isDeleted: { $ne: true }
  };
...
/**
 * Format template response from database record
 */
const formatTemplateResponse = (template: any): INoteTemplate => {
  return {
...
/**
 * Calculate word count for template content
 */
const calculateWordCount = (content: INoteContentBlock[]): number => {
  let wordCount = 0;
...
/**
 * Process template content with custom values
 */
