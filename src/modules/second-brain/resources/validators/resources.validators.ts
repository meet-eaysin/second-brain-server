...
export const updateResourceSchema = z.object({
  ...
  category: z.enum(EResourceCategory).optional(),
  status: z.enum(EResourceStatus).optional(),
  accessLevel: z.enum(EResourceAccessLevel).optional(),
  url: z.string().url('Invalid URL format').optional(),
  filePath: z.string().max(500, 'File path too long').optional(),
  content: z.string().optional(),
...
export const getResourcesQuerySchema = z.object({
  ...
  type: z
    .array(z.enum(EResourceType))
    .or(z.string().transform(val => val.split(',').map(s => s.trim() as EResourceType)))
    .optional(),
  category: z
    .array(z.enum(EResourceCategory))
    .or(z.string().transform(val => val.split(',').map(s => s.trim() as EResourceCategory)))
    .optional(),
  status: z
    .array(z.enum(EResourceStatus))
    .or(z.string().transform(val => val.split(',').map(s => s.trim() as EResourceStatus)))
    .optional(),
  accessLevel: z
    .array(z.enum(EResourceAccessLevel))
    .or(z.string().transform(val => val.split(',').map(s => s.trim() as EResourceAccessLevel)))
    .optional(),
  tags: z
    .array(z.string())
  ...
  limit: z
    ...
    .or(z.string().transform(val => parseInt(val, 10))),
  sortBy: z
    .enum(['title', 'createdAt', 'updatedAt', 'lastAccessedAt', 'viewCount', 'rating'])
    .default('updatedAt'),
