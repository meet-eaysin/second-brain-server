...
function buildResourcesQuery(params: IResourceQueryParams, userId: string): any {
  const query: any = {
    isDeleted: { $ne: true }
  };
...
function formatResourceResponse(record: any): IResource {
  return {
...
async function addChildResource(parentId: string, childId: string, userId: string): Promise<void> {
...
export const resourcesService = {
  ...
  createResource: async (data: ICreateResourceRequest, userId: string): Promise<IResource> => {
    try {
      ...
      if (data.parentResourceId) {
        await addChildResource(data.parentResourceId, savedRecord._id as string, userId);
      }
      ...
      return formatResourceResponse(savedRecord);
    } catch (error: any) {
  ...
  getResources: async (
    ...
    try {
      const query = buildResourcesQuery(params, userId);
      ...
      const formattedResources = resources.map(resource => formatResourceResponse(resource));
  ...
  getResourceById: async (id: string, userId: string): Promise<IResource> => {
    try {
  ...
  updateResource: async (
    ...
    try {
      ...
      const updatedResource = await RecordModel.findByIdAndUpdate(id, updateData, {
        new: true,
        runValidators: true
      }).exec();
      ...
      await DatabaseModel.findByIdAndUpdate(updatedResource.databaseId, {
        lastActivityAt: new Date()
      });

      return formatResourceResponse(updatedResource);
    } catch (error: any) {
      if (error.statusCode) throw error;
  ...
  deleteResource: async (id: string, userId: string, permanent: boolean = false): Promise<void> => {
    try {
      const resource = await RecordModel.findOne({
        _id: id,
        isDeleted: { $ne: true }
      ...
      await DatabaseModel.findByIdAndUpdate(resource.databaseId, {
        $inc: { recordCount: -1 },
        lastActivityAt: new Date()
      });
    } catch (error: any) {
