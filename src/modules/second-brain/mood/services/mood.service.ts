  ...
  try {
    ...
    if (!hasPermission) {
      throw createForbiddenError(
        'Insufficient permissions to create mood entries in this database'
      );
    }
    ...
    const aiInsights = await generateAIInsights(data, userId);
    ...
    analyzePatterns(userId, data.databaseId).catch(console.error);

    return formatMoodEntryResponse(savedRecord);
  } catch (error: any) {
...
export const getMoodEntries = async (
  ...
  try {
    const query = buildMoodQuery(params, userId);
    ...
    const formattedEntries = entries.map(entry => formatMoodEntryResponse(entry));
 ...
 */
export const getMoodEntryById = async (id: string, userId: string): Promise<IMoodEntry> => {
  try {
    ...
    return formatMoodEntryResponse(entry);
  } catch (error: any) {
    if (error.statusCode) throw error;
...
export const updateMoodEntry = async (
  ...
  try {
    ...
    if (data.customTriggers !== undefined)
      updateData['properties.Custom Triggers'] = data.customTriggers;
    ...
    if (data.exerciseMinutes !== undefined)
      updateData['properties.Exercise Minutes'] = data.exerciseMinutes;
    ...
    if (data.significantEvents !== undefined)
      updateData['properties.Significant Events'] = data.significantEvents;
    ...
    if (data.tomorrowGoals !== undefined)
      updateData['properties.Tomorrow Goals'] = data.tomorrowGoals;
    if (data.socialInteractions !== undefined)
      updateData['properties.Social Interactions'] = data.socialInteractions;
    if (data.mediaConsumed !== undefined)
      updateData['properties.Media Consumed'] = data.mediaConsumed;
    if (data.habitsCompleted !== undefined)
      updateData['properties.Habits Completed'] = data.habitsCompleted;
    if (data.routineAdherence !== undefined)
      updateData['properties.Routine Adherence'] = data.routineAdherence;
    if (data.predictedMood !== undefined)
      updateData['properties.Predicted Mood'] = data.predictedMood;
    ...
    const updatedEntry = await RecordModel.findByIdAndUpdate(id, updateData, {
      new: true,
      runValidators: true
    }).exec();
    ...
    await DatabaseModel.findByIdAndUpdate(updatedEntry.databaseId, { lastActivityAt: new Date() });

    return formatMoodEntryResponse(updatedEntry);
  } catch (error: any) {
    if (error.statusCode) throw error;
...
export const deleteMoodEntry = async (
  ...
  try {
    ...
    await DatabaseModel.findByIdAndUpdate(entry.databaseId, {
      $inc: { recordCount: -1 },
      lastActivityAt: new Date()
    });
  } catch (error: any) {
...
export const getMoodAnalytics = async (
  params: IMoodAnalyticsRequest,
  userId: string
): Promise<IMoodAnalytics> => {
  try {
...
/**
 * Generate AI insights for mood entry (placeholder)
 */
const generateAIInsights = async (data: ICreateMoodEntryRequest, userId: string): Promise<any> => {
...
/**
 * Analyze mood patterns (placeholder)
 */
const analyzePatterns = async (userId: string, databaseId: string): Promise<void> => {
...
/**
 * Build mood query from parameters
 */
const buildMoodQuery = (params: IMoodQueryParams, userId: string): any => {
  const query: any = {
    isDeleted: { $ne: true }
  };
...
/**
 * Format mood entry response from database record
 */
const formatMoodEntryResponse = (record: any): IMoodEntry => {
  return {
