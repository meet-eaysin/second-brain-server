...
export class ProjectsService {
  ...
  async createProject(data: ICreateProjectRequest, userId: string): Promise<IProject> {
    try {
      ...
      return this.formatProjectResponse(savedRecord);
    } catch (error: any) {
  ...
  async getProjects(params: IProjectQueryParams, userId: string): Promise<{
    projects: IProject[];
    total: number;
    page: number;
    limit: number;
    ...
    try {
      const query = this.buildProjectsQuery(params, userId);
      ...
      const formattedProjects = projects.map(project => this.formatProjectResponse(project));
  ...
  async updateProject(id: string, data: IUpdateProjectRequest, userId: string): Promise<IProject> {
    try {
      const project = await RecordModel.findOne({
        _id: id,
        isDeleted: { $ne: true }
      ...
      const updatedProject = await RecordModel.findByIdAndUpdate(
        id,
        updateData,
  ...
  private buildProjectsQuery(params: IProjectQueryParams, userId: string): any {
    const query: any = {
      isDeleted: { $ne: true }
    };
  ...
  private formatProjectResponse(record: any): IProject {
    return {
