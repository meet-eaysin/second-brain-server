...
async function markHabitCompleted(
  ...
  // Update habit streak
  await updateHabitStreak(habitId, date, true);

  // Store entry in habit's completion history
  await storeHabitEntry(habitId, entry, userId);
 ...
 */
async function markHabitIncomplete(habitId: string, date: string, userId?: string): Promise<void> {
  const habit = await RecordModel.findById(habitId);
  if (!habit) {
    throw createNotFoundError('Habit not found');
  }
  ...
  // Update habit streak
  await updateHabitStreak(habitId, date, false);

  // Remove or update entry
  await removeHabitEntry(habitId, date, userId);
}
 ...
 */
async function calculateHabitStats(habitId: string): Promise<IHabitStats> {
  const habit = await RecordModel.findById(habitId);
  if (!habit) {
    throw createNotFoundError('Habit not found');
  }
  ...
  const expectedCompletions = calculateExpectedCompletions(frequency, daysSinceStart);
  ...
  // Get last seven days
  const lastSevenDays = getLastSevenDaysCompletion(completionHistory);

  // Get monthly progress
  const monthlyProgress = getMonthlyProgress(completionHistory);
 ...
 */
async function getHabitsDueToday(userId: string): Promise<any[]> {
  ...
  for (const habit of habits) {
    ...
    if (isHabitDueToday(frequency, completionHistory, today)) {
 ...
 * Update habit streak
 */
async function updateHabitStreak(habitId: string, date: string, completed: boolean): Promise<void> {
  const habit = await RecordModel.findById(habitId);
  if (!habit) return;
  ...
  const streak = calculateStreak(completionHistory, frequency, date, completed);
 ...
 * Store habit entry
 */
async function storeHabitEntry(
  habitId: string,
  entry: IHabitEntry,
  userId?: string
): Promise<void> {
 ...
 * Remove habit entry
 */
async function removeHabitEntry(habitId: string, date: string, userId?: string): Promise<void> {
  await RecordModel.findByIdAndUpdate(habitId, {
 ...
 */
function calculateExpectedCompletions(frequency: string, days: number): number {
  switch (frequency) {
    case 'daily':
      return days;
    case 'weekly':
 ...
 */
function getLastSevenDaysCompletion(completionHistory: IHabitEntry[]): boolean[] {
  const result = [];
  const today = new Date();
 ...
 * Get monthly progress
 */
function getMonthlyProgress(completionHistory: IHabitEntry[]): { [month: string]: number } {
...
/**
 * Habits service object containing all habit-related operations
 */
