...
export const exportDatabase = async (
  databaseId: string,
  userId: string,
  options: TDatabaseExportOptions
  ...
  try {
    ...
    const recordsResult: TRecordsListResponse = await databaseService.getRecords(
      databaseId,
      userId,
      queryParams
    );
...
export const importData = async (
  ...
  options: TDatabaseImportOptions
  ...
  try {
      ...
      switch (options.format) {
        ...
        case 'json':
          ...
          if (Array.isArray(parsedJson)) {
            ...
            throw createImportDataInvalidError(
              'JSON must contain an array of records or an object with a records array'
            );
          }
          break;
        default:
    ...
    if (options.createMissingProperties) {
      for (const columnName of columnNames) {
        ...
        const detectedType = detectPropertyType(
          sampleValue,
        ...
        for (const [columnName, value] of Object.entries(row)) {
          ...
          if (!propertyId) {
            const property = database.properties.find(
              p => p.name.toLowerCase() === columnName.toLowerCase()
            );
