...
const calculateRollupValue = async (
  recordId: string,
  ...
  // Get related records
  const relatedRecords = await getRelatedRecordsForRollup(
    recordId,
    rollupConfig.relationPropertyId
  );

  if (relatedRecords.length === 0) {
    return getEmptyRollupValue(rollupConfig.rollupFunction);
  }

  // Apply filters if specified
  const filteredRecords = rollupConfig.filters
    ? applyFilters(relatedRecords, rollupConfig.filters)
    : relatedRecords;

  // Extract values from the rollup property
  const values = extractPropertyValues(filteredRecords, rollupProperty.name);

  // Calculate rollup based on function
  return calculateRollupFunction(
...
const getRelatedRecordsForRollup = async (
  recordId: string,
  relationPropertyId: string
): Promise<any[]> => {
...
const applyFilters = (
  ...
  return records.filter(record => {
    return filters.every(filter => {
      ...
      return evaluateFilterCondition(propertyValue, filter.condition, filter.value);
    });
  });
};

const evaluateFilterCondition = (value: any, condition: string, filterValue: any): boolean => {
  switch (condition) {
    case 'equals':
...
const extractPropertyValues = (records: any[], propertyName: string): any[] => {
  return records
    .map(record => record.properties?.[propertyName])
    .filter(value => value !== null && value !== undefined);
};

const calculateRollupFunction = (
...
const getEmptyRollupValue = (func: ERollupFunction): any => {
  switch (func) {
    case ERollupFunction.COUNT:
    case ERollupFunction.COUNT_VALUES:
    case ERollupFunction.COUNT_UNIQUE:
...
const updateDependentRollups = async (recordId: string): Promise<void> => {
  ...
  for (const rollupProperty of rollupProperties) {
    if (rollupProperty.config.rollupPropertyId && rollupProperty.config.relationPropertyId) {
      ...
      const affectedRecords = await findRecordsAffectedByChange(
        recordId,
      ...
      for (const affectedRecord of affectedRecords) {
        const rollupValue = await calculateRollupValue(
...
const findRecordsAffectedByChange = async (
...
const recalculateAllRollups = async (databaseId: string): Promise<void> => {
...
export const rollupService = {
  ...
  updateDependentRollups,
  recalculateAllRollups
};
