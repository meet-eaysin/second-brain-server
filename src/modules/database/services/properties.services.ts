...
const createProperty = async (
  ...
  const maxOrder = await PropertyModel.findOne({ databaseId }).sort({ order: -1 }).select('order');
  ...
  validatePropertyConfig(property.toObject() as IProperty);
...
const getPropertyById = async (databaseId: string, propertyId: string): Promise<IProperty> => {
  const database = await DatabaseModel.findById(databaseId);
...
const updateProperty = async (
  ...
  if (data.config) {
    ...
    validatePropertyConfig(updatedProperty as IProperty);
  }
...
const reorderProperties = async (
  ...
  return getProperties(databaseId, userId, true);
};
...
const duplicateProperty = async (
  ...
  const originalProperty = await getPropertyById(databaseId, propertyId);
  ...
  return createProperty(databaseId, duplicateData, userId);
};
...
const togglePropertyVisibility = async (
  databaseId: string,
  propertyId: string,
  viewId: string,
  userId: string
  ...
  const property = await getPropertyById(databaseId, propertyId);
...
const getPropertyCalculations = async (databaseId: string, propertyId: string): Promise<any> => {
  const property = await getPropertyById(databaseId, propertyId);
  ...
  if (property.type === EPropertyType.NUMBER) {
    ...
    if (numericValues.length > 0) {
      ...
      calculations.median = calculateMedian(numericValues);
    }
  }
...
const validatePropertyConfig = (property: IProperty): void => {
  ...
  switch (type) {
    ...
    case EPropertyType.RELATION:
      ...
      validateRelationDatabase(config.relationDatabaseId);
      break;
    ...
    case EPropertyType.ROLLUP:
      ...
      validateRollupConfiguration(config);
      break;
...
const validateRelationDatabase = async (databaseId: string): Promise<void> => {
  const database = await DatabaseModel.findById(databaseId);
  if (!database) {
...
const validateRollupConfiguration = async (config: any): Promise<void> => {
  ...
  validateRollupFunction(config.rollupFunction, rollupProperty.type);
};

const validateRollupFunction = (rollupFunction: string, propertyType: EPropertyType): void => {
