...
export class BlocksService {
  ...
  async addBlock(
    ...
    const record = await this.verifyRecordAccess(databaseId, recordId, userId);

    // Validate block content
    const validation = this.validateBlockContent(data.type, data.content);
    ...
    return this.formatBlockResponse(recordContent);
  }

  // Get all blocks for a record
  async getBlocks(
    databaseId: string,
    recordId: string,
    userId: string,
    ...
    // Apply search query
    if (options.query) {
      blocks = blocks.filter(block => this.blockMatchesQuery(block, options.query!));
    }
    ...
    return {
      blocks: paginatedBlocks.map(block => this.formatBlockResponse(block)),
      total: blocks.length,
      hasMore,
      nextCursor
    };
  ...
  async updateBlock(
    ...
    if (data.content) {
      ...
      const validation = this.validateBlockContent(blockType, data.content);
      if (!validation.isValid) {
        throw createAppError(`Block validation failed: ${validation.errors.join(', ')}`, 400);
      }
    }
        ...
        switch (operation.operation) {
          ...
          case 'delete':
            if (operation.blockId) {
              await this.deleteBlock(databaseId, recordId, operation.blockId, userId);
            }
            break;
          case 'move':
            if (operation.blockId && operation.data) {
  ...
  // Helper methods
  private async verifyRecordAccess(databaseId: string, recordId: string, userId: string) {
  ...
  private blockMatchesQuery(block: IRecordContent, query: string): boolean {
  ...
  private extractRichTextFromBlock(block: IRecordContent): IRichTextContent[][] {
  ...
  private formatBlockResponse(block: IRecordContent): IBlockResponse {
    ...
    if (block.children && block.children.length > 0) {
      (cleanBlock as any).children = block.children.map(child => this.formatBlockResponse(child));
    }

    return cleanBlock as IBlockResponse;
  }

  // Apply rich text formatting to block content
  async applyRichFormatting(
    recordId: string,
    blockId: string,
    start: number,
    end: number,
  ...
  async getBlockStatistics(recordId: string, blockId: string): Promise<any> {
    const record = await RecordModel.findById(recordId);
    if (!record) {
      throw createNotFoundError('Record not found');
    }
  ...
  // Convert block to different format
  async convertBlockFormat(
    recordId: string,
    blockId: string,
