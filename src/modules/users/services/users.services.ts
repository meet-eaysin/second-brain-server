...
export const createUser = async (userData: TUserCreateRequest): Promise<TUser> => {
    try {
        ...
        // Validate password for local auth provider
        ...
        if (existingUser) {
            ...
            if (existingUser.username === userData.username) {
                throw createUsernameExistsError(userData.username);
            }
        }
...
export const updateUser = async (id: string, updateData: TUserUpdateRequest): Promise<TUser | null> => {
    try {
        ...
        // Check for existing user with same email or username (excluding current user)
