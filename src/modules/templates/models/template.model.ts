...
export type TTemplateModel = Model<TTemplateDocument> & {
  findByCategory(category: ETemplateCategory): mongoose.Query<TTemplateDocument[], TTemplateDocument>;
...
TemplateSchema.statics.findFeatured = function() {
  return this.find({ isFeatured: true, isDeleted: { $ne: true } })
    .sort({ usageCount: -1, rating: -1 });
};

TemplateSchema.statics.findOfficial = function() {
  return this.find({ isOfficial: true, isDeleted: { $ne: true } })
    .sort({ usageCount: -1, rating: -1 });
};

TemplateSchema.statics.findPublic = function() {
  return this.find({ access: ETemplateAccess.PUBLIC, isDeleted: { $ne: true } })
    .sort({ usageCount: -1, rating: -1 });
};

TemplateSchema.statics.findByUser = function(userId: string) {
  return this.find({ createdBy: userId, isDeleted: { $ne: true } })
    .sort({ updatedAt: -1 });
};
...
export const TemplateModel = mongoose.model<TTemplateDocument, TTemplateModel>('Template', TemplateSchema);
