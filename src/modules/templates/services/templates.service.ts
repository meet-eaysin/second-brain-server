...
const createTemplate = async (
  ...
  const templateDataWithBooleans = {
    ...safeTemplateData,
    isOfficial: toBooleanSafe(safeTemplateData.isOfficial),
    isFeatured: toBooleanSafe(safeTemplateData.isFeatured),
...
// Get template by ID
const getTemplate = async (templateId: string, userId?: string): Promise<TTemplateDocument> => {
...
const searchTemplates = async (
  ...
  const accessibleTemplates = templates.filter(template => canAccessTemplate(template, userId));

  // Calculate facets
  const facets = calculateSearchFacets(accessibleTemplates);

  return {
    templates: accessibleTemplates.map((t: TTemplateDocument) => formatTemplate(t)),
...
const getFeaturedTemplates = async (limit = 10): Promise<TTemplateDocument[]> => {
  const templates = await TemplateModel.findFeatured().limit(limit);
  return templates.map((t: TTemplateDocument) => formatTemplate(t));
};

// Get official templates
const getOfficialTemplates = async (limit = 20): Promise<TTemplateDocument[]> => {
...
const getTemplatesByModule = async (
...
const getUserTemplates = async (userId: string): Promise<TTemplateDocument[]> => {
  const templates = await TemplateModel.findByUser(userId);
  return templates.map((t: TTemplateDocument) => formatTemplate(t));
};

// Apply row template
const applyRowTemplate = async (
  templateId: string,
  databaseId: string,
  ...
  const processedData = applyConditionalLogic(recordData, template.conditionalLogic || []);

  // Apply auto-fill rules
  const finalData = applyAutoFillRules(processedData, template.autoFillRules || []);
  ...
  await trackTemplateUsage(templateId, userId, undefined, 'manual');

  return record;
};

// Apply database template
const applyDatabaseTemplate = async (
  templateId: string,
  workspaceId: string,
  ...
  // Track template usage
  await trackTemplateUsage(templateId, userId, workspaceId, 'manual');
...
const applyWorkspaceTemplate = async (
  templateId: string,
  ...
  // Track template usage
  await trackTemplateUsage(templateId, userId, workspace.id.toString(), 'manual');
...
const updateTemplate = async (
  ...
  if ('isOfficial' in updates && updates.isOfficial !== undefined) {
    updatesWithBooleans.isOfficial = toBooleanSafe(updates.isOfficial);
  }
  if ('isFeatured' in updates && updates.isFeatured !== undefined) {
    updatesWithBooleans.isFeatured = toBooleanSafe(updates.isFeatured);
  }
...
// Delete template
const deleteTemplate = async (templateId: string, userId: string): Promise<void> => {
  const template = await TemplateModel.findById(templateId);
  if (!template) {
    throw createNotFoundError('Template not found');
  }
...
// Rate template
const rateTemplate = async (templateId: string, rating: number, userId: string): Promise<void> => {
...
const getTemplateAnalytics = async (templateId: string): Promise<ITemplateAnalytics> => {
  const usageStats = await TemplateUsageModel.getUsageStats(templateId);

  // Get usage by month for the last 12 months
  const usageByMonth = await getUsageByMonth(templateId);

  // Get top users
  const topUsers = await getTopUsers(templateId);
...
const getUserTemplateHistory = async (userId: string, limit = 20): Promise<any[]> => {
  return TemplateUsageModel.getUserTemplateHistory(userId, limit);
};

// Get popular templates
const getPopularTemplates = async (limit = 10): Promise<any[]> => {
  return TemplateUsageModel.getPopularTemplates(limit);
};

// Private helper methods
const formatTemplate = (template: TTemplateDocument): TTemplateDocument => {
...
const canAccessTemplate = (template: TTemplateDocument, userId?: string): boolean => {
...
const applyConditionalLogic = (data: Record<string, any>, logic: any[]): Record<string, any> => {
  // Apply conditional logic rules
  // This is a simplified implementation
  return data;
};

const applyAutoFillRules = (data: Record<string, any>, rules: any[]): Record<string, any> => {
  // Apply auto-fill rules
  // This is a simplified implementation
  return data;
};

const calculateSearchFacets = (
  templates: TTemplateDocument[]
): {
...
const getUsageByMonth = async (templateId: string): Promise<{ [month: string]: number }> => {
