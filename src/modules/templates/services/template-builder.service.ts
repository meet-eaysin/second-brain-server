...
const createRowTemplateFromRecord = async (
  recordId: string,
  templateData: {
    name: string;
    description: string;
  ...
  const rowTemplate = await templatesService.createTemplate(
    {
      ...
      templateData: {
        ...
        defaultValues,
        requiredProperties: extractRequiredProperties(database),
        conditionalLogic: [],
        autoFillRules: []
      }
    },
...
const createDatabaseTemplateFromDatabase = async (
  databaseId: string,
  templateData: {
    name: string;
    description: string;
  ...
  const templateViews: ITemplateView[] = views.map(view => ({
    ...
    filters: convertFilterGroupToTemplateFilters(view.filters),
    sorts: convertSortsToTemplateSorts(view.sorts),
    groups: convertGroupToTemplateGroups(view.config?.group)
  }));
...
const createWorkspaceTemplateFromWorkspace = async (
  workspaceId: string,
  templateData: {
    name: string;
    description: string;
  ...
  for (const database of databases) {
    const dbTemplate = await createDatabaseTemplateFromDatabase(
  ...
  const workspaceTemplate = await templatesService.createTemplate(
    {
      ...
      templateData: {
        ...
        onboardingFlow: generateDefaultOnboardingFlow(uniqueModules)
      }
    },
    userId
  );
...
const generateTemplateFromPrompt = async (
  ...
  const templateName = extractNameFromPrompt(prompt);
  const templateDescription = extractDescriptionFromPrompt(prompt);
  const category = inferCategoryFromPrompt(prompt);

  switch (templateType) {
    case ETemplateType.ROW:
      return generateRowTemplateFromPrompt(
        prompt,
        moduleType,
        {
          name: templateName,
    ...
    case ETemplateType.DATABASE:
      return generateDatabaseTemplateFromPrompt(
        prompt,
        moduleType,
        {
          name: templateName,
    ...
    case ETemplateType.WORKSPACE:
      return generateWorkspaceTemplateFromPrompt(
        prompt,
        {
          name: templateName,
          description: templateDescription,
...
const analyzeDatabaseForTemplates = async (databaseId: string): Promise<any> => {
  const database = await DatabaseModel.findById(databaseId);
  if (!database) {
    throw createNotFoundError('Database not found');
  }
  ...
  const analysis = {
    ...
    commonPatterns: findCommonPatterns(records),
    suggestedTemplates: suggestTemplatesFromAnalysis(database, records, properties),
    optimizationSuggestions: generateOptimizationSuggestions(database, records, properties)
  };
...
const validateTemplateData = async (
  ...
  switch (templateType) {
    case ETemplateType.ROW:
      validateRowTemplate(templateData, validation);
      break;
    case ETemplateType.DATABASE:
      validateDatabaseTemplate(templateData, validation);
      break;
    case ETemplateType.WORKSPACE:
      validateWorkspaceTemplate(templateData, validation);
      break;
  }
...
const extractRequiredProperties = (database: any): string[] => {
  // Extract required properties from database schema
  return [];
};

const generateDefaultOnboardingFlow = (modules: EDatabaseType[]): any[] => {
  const steps = [
    {
      id: 'welcome',
...
const inferCategoryFromPrompt = (prompt: string): ETemplateCategory => {
...
const generateRowTemplateFromPrompt = async (
  prompt: string,
  moduleType: EDatabaseType,
  templateData: any,
  userId: string
  ...
  const defaultValues = generateDefaultValuesFromPrompt(prompt, moduleType);
  ...
  return templatesService.createTemplate(
    ...
    userId
  ) as Promise<IRowTemplate>;
};

const generateDatabaseTemplateFromPrompt = async (
  prompt: string,
  moduleType: EDatabaseType,
  templateData: any,
  userId: string
  ...
  const properties = generatePropertiesFromPrompt(prompt, moduleType);
  const views = generateViewsFromPrompt(prompt, moduleType);
  ...
  return templatesService.createTemplate(
    ...
    userId
  ) as Promise<IDatabaseTemplate>;
};

const generateWorkspaceTemplateFromPrompt = async (
  prompt: string,
  templateData: any,
  userId: string
): Promise<IWorkspaceTemplate> => {
  // Infer modules from prompt
  const modules = inferModulesFromPrompt(prompt);
  ...
  return templatesService.createTemplate(
    {
      ...
      templateData: {
        ...
        onboardingFlow: generateDefaultOnboardingFlow(modules)
      }
    },
    userId
  ) as Promise<IWorkspaceTemplate>;
};

const generateDefaultValuesFromPrompt = (
  prompt: string,
  moduleType: EDatabaseType
): Record<string, any> => {
...
const generatePropertiesFromPrompt = (
  prompt: string,
  moduleType: EDatabaseType
): ITemplateProperty[] => {
...
const generateViewsFromPrompt = (prompt: string, moduleType: EDatabaseType): ITemplateView[] => {
  // Generate basic views - would use AI in production
  return [];
};

const inferModulesFromPrompt = (prompt: string): EDatabaseType[] => {
...
const convertFilterGroupToTemplateFilters = (filterGroup: IFilterGroup): ITemplateFilter[] => {
...
const convertSortsToTemplateSorts = (sorts: ISortConfig[]): ITemplateSort[] => {
  if (!sorts) {
    return [];
  }
...
const convertGroupToTemplateGroups = (groupConfig?: IGroupConfig): ITemplateGroup[] => {
  if (!groupConfig) {
    return [];
  }
...
const findCommonPatterns = (records: any[]): any[] => {
  // Analyze records for common patterns
  return [];
};

const suggestTemplatesFromAnalysis = (database: any, records: any[], properties: any[]): any[] => {
...
const validateRowTemplate = (templateData: any, validation: any): void => {
...
const validateDatabaseTemplate = (templateData: any, validation: any): void => {
  if (!templateData.moduleType) {
...
const validateWorkspaceTemplate = (templateData: any, validation: any): void => {
