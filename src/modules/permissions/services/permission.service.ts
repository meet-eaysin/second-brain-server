...
import { EWorkspaceMemberRole } from '@/modules/workspace/types/workspace.types';
...
const hasPermission = async (
  ...
  try {
    ...
    if (directPermission && hasRequiredLevel(directPermission.level, requiredLevel)) {
      return true;
    }

    // 2. Check template-specific permissions
    if (resourceType === EShareScope.TEMPLATE) {
      const templatePermission = await checkTemplatePermission(resourceId, userId, requiredLevel);
      if (templatePermission) return true;
    }

    // 3. Check workspace-level permissions (for workspace-scoped resources)
    if (resourceType === EShareScope.DATABASE || resourceType === EShareScope.RECORD) {
      const workspacePermission = await checkWorkspacePermission(
        resourceType,
        resourceId,
        userId,
        requiredLevel
    ...
    if (resourceType === EShareScope.RECORD) {
      const inheritedPermission = await checkInheritedPermission(resourceId, userId, requiredLevel);
    ...
    if (publicPermission && hasRequiredLevel(publicPermission.level, requiredLevel)) {
      return true;
    }
...
const checkWorkspacePermission = async (
  ...
  try {
    ...
    return hasRequiredLevel(memberPermissionLevel, requiredLevel);
  } catch (error) {
...
const checkTemplatePermission = async (
  templateId: string,
  userId: string,
  requiredLevel: EPermissionLevel
): Promise<boolean> => {
  try {
    ...
    if (template.access === ETemplateAccess.PUBLIC) {
      ...
      return hasRequiredLevel(EPermissionLevel.COMMENT, requiredLevel);
    }
    ...
    if (
      template.access === ETemplateAccess.TEAM ||
      template.access === ETemplateAccess.ORGANIZATION
    ) {
      if (!userId || template.createdBy.toString() !== userId) {
        return false;
      }
...
const checkInheritedPermission = async (
  recordId: string,
  userId: string,
  requiredLevel: EPermissionLevel
): Promise<boolean> => {
  try {
    ...
    return hasPermission(EShareScope.DATABASE, record.databaseId, userId, requiredLevel);
  } catch (error) {
...
const getResourcePermissions = async (
  ...
  const canView = await hasPermission(resourceType, resourceId, requesterId, EPermissionLevel.EDIT);
...
const getUserPermissions = async (userId: string): Promise<IPermission[]> => {
  const permissions = await PermissionModel.find({
    type: EPermissionType.USER,
    userId,
    isActive: true,
...
const hasCapability = async (
  ...
  if (!permission) {
    ...
    if (resourceType === EShareScope.TEMPLATE) {
      return checkTemplateCapability(resourceId, userId, capability);
    }

    // Check workspace-level capability
    return checkWorkspaceCapability(resourceType, resourceId, userId, capability);
  }

  return permission[capability] || false;
};

const checkWorkspaceCapability = async (
  resourceType: EShareScope,
  resourceId: string,
  userId: string,
  capability: string
  ...
  try {
    ...
    const capabilityMap: Record<string, boolean> = {
      ...
      canManagePermissions: [EWorkspaceMemberRole.OWNER, EWorkspaceMemberRole.ADMIN].includes(
        member.role
      )
    };
...
const checkTemplateCapability = async (
  templateId: string,
  userId: string,
  capability: string
): Promise<boolean> => {
