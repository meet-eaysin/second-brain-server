...
const validateViewAccess = async (
  viewId: string,
  userId: string,
  requiredLevel: EPermissionLevel = EPermissionLevel.READ,
  options: IPermissionValidationOptions = {}
...
const validateTemplateAccess = async (
  templateId: string,
  userId: string,
  requiredLevel: EPermissionLevel = EPermissionLevel.READ,
  options: IPermissionValidationOptions = {}
...
const validateMultiplePermissions = async (
  checks: Array<{
    resourceType: EShareScope;
    resourceId: string;
    userId: string;
...
const validateOwnership = async (
  ...
  try {
    ...
    if (!isOwner && options.throwOnFailure !== false) {
      throw createForbiddenError(options.customErrorMessage || 'You do not own this resource');
    }
...
const calculateEffectivePermissions = async (
  ...
  // Check ownership
  const isOwner = await validateOwnership(resourceType, resourceId, userId, {
