...
export const createLocalAccountError = (): TAppError => {
    return createUnauthorizedError(AUTH_ERROR_MESSAGES.LOCAL_ACCOUNT);
};

// Token errors
export const createTokenExpiredError = (): TAppError => {
    return createUnauthorizedError(AUTH_ERROR_MESSAGES.TOKEN_EXPIRED);
};

export const createTokenInvalidError = (): TAppError => {
    return createUnauthorizedError(AUTH_ERROR_MESSAGES.TOKEN_INVALID);
};

export const createTokenMalformedError = (): TAppError => {
    return createUnauthorizedError(AUTH_ERROR_MESSAGES.TOKEN_MALFORMED);
};

export const createTokenMissingError = (): TAppError => {
    return createUnauthorizedError(AUTH_ERROR_MESSAGES.TOKEN_MISSING);
};
...
export const createPasswordWeakError = (): TAppError => {
    return createValidationError(AUTH_ERROR_MESSAGES.PASSWORD_WEAK, {
        field: 'password',
        code: 'WEAK_PASSWORD'
    });
};
...
export const createOAuthProfileInvalidError = (): TAppError => {
    return createValidationError(AUTH_ERROR_MESSAGES.OAUTH_PROFILE_INVALID, {
...
// Registration errors
export const createEmailAlreadyExistsError = (email: string): TAppError => {
    return createConflictError(`${AUTH_ERROR_MESSAGES.EMAIL_ALREADY_EXISTS}: ${email}`);
};

export const createUsernameAlreadyExistsError = (username: string): TAppError => {
    return createConflictError(`${AUTH_ERROR_MESSAGES.USERNAME_ALREADY_EXISTS}: ${username}`);
};

export const createRegistrationFailedError = (reason?: string): TAppError => {
    const message = reason
        ? `${AUTH_ERROR_MESSAGES.REGISTRATION_FAILED}: ${reason}`
...
export const createInvalidUsernameFormatError = (): TAppError => {
    return createValidationError(AUTH_ERROR_MESSAGES.INVALID_USERNAME_FORMAT, {
        field: 'username',
        code: 'INVALID_FORMAT'
    });
};
...
export const createInvalidNameFormatError = (field: string): TAppError => {
    return createValidationError(AUTH_ERROR_MESSAGES.INVALID_NAME_FORMAT, {
        field,
        code: 'INVALID_FORMAT'
    });
};
...
export const createAuthorizationFailedError = (): TAppError => {
    return createForbiddenError(AUTH_ERROR_MESSAGES.AUTHORIZATION_FAILED);
};

export const createSessionExpiredError = (): TAppError => {
    return createUnauthorizedError(AUTH_ERROR_MESSAGES.SESSION_EXPIRED);
};

export const createOperationNotAllowedError = (operation: string): TAppError => {
    return createForbiddenError(`${AUTH_ERROR_MESSAGES.OPERATION_NOT_ALLOWED}: ${operation}`);
};
